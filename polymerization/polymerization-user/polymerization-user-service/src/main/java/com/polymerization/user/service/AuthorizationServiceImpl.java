...
import com.polymerization.user.convert.AuthorizationPrivilegeConvert;
import com.polymerization.user.convert.AuthorizationRoleConvert;
...
//获取员工用户名，即账号的用户名
    ...
    private List<AccountRoleDTO> queryAccountRole(String username, Long tenantId) {
        List<AccountRole> accountRoles = accountRoleMapper.selectList(new QueryWrapper<AccountRole>().lambda()
                .eq(AccountRole::getUsername, username).eq(AccountRole::getTenantId, tenantId));
        List<AccountRoleDTO> accountRoleDTOS = new ArrayList<>();
        for (AccountRole accountRole : accountRoles) {
